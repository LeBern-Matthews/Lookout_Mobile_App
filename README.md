...
## Features
### 1. Onboarding Flow
- A structured 9-screen wizard to capture user demographics, location, and risk factors.
- Auto-detects the user's country based on device location to personalize emergency contacts.
- Data is securely stored locally and used to customize the preparedness checklist.
...
- Logs key user behaviors including onboarding completion, custom contact creation, and emergency calls for funnel analysis.
...
3. Create a `.env` file in the root directory and add your Firebase configuration variables (see `firebase_options.dart` for required keys).
...
The app uses the **Provider** pattern for state management across five main providers:
...
- `UserPreferencesProvider` — manages user data captured during onboarding
...
6. [location](https://pub.dev/packages/location) & [geocoding](https://pub.dev/packages/geocoding) — Device location access and country detection
7. [firebase_core](https://pub.dev/packages/firebase_core) & [firebase_analytics](https://pub.dev/packages/firebase_analytics) — Firebase initialization and usage tracking
8. [flutter_dotenv](https://pub.dev/packages/flutter_dotenv) — Secure environment variable management
9. [google_maps_flutter](https://pub.dev/packages/google_maps_flutter) — Google Maps integration (planned for upcoming map feature)
